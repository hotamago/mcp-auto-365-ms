...
def read_sharepoint_link(url: str, max_depth: int = 2) -> str:
    ...
    - If the link is a folder: returns directory tree, child files, sizes, modified dates, and authors.
    - If the link is a document: returns metadata (SharePoint path, size, author, created/modified date, version changelog). Does NOT convert or destroy document formatting.
    """
    try:
...
def download_sharepoint_link(url_or_guid: str, target_dir: str = "docs/sharepoint") -> str:
    ...
    - Preserves exact binary format (.docx, .xlsx, .pptx, .pdf, images, charts, layout).
    - If a folder URL is provided, recursively downloads all documents preserving subfolder structure.
    - If a document URL or GUID is provided, downloads that specific file.
    """
    try:
